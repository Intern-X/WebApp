...
export default function generateReferralRequest(alumni , userInfo, callback) {
    ...
    "messages": [
      {
        "role": "user",
        "content": `
        Write a concise, semi-formal, high-quality job referral request email introducing myself to alumnus ${alumni.name} at ${alumni.company}, 
        in first person. Each sentence should be under 10 to 15 words. Focus on politely requesting a job referral. 
        ...
        Make the subject some sort of referral request invitation.
  ...
  llamaAPI.run(apiRequestJson)
    .then(response => {
      console.log("Referral Request Response:", response);
