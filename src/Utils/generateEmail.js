...
const baseURL = "https://projx-server-70cdfb12f7a8.herokuapp.com";
const llamaClient = new LlamaStackClient({ apiKey: apiToken, baseURL });


async function testConnection() {
  try {
    const status = await llamaClient.inspect.health();
    console.log("Connection Status Success:", status);
  }
  catch (error) {
...
export default function generateEmail(recruiter, userInfo, callback) {
  ...
  llamaClient.inference.chatCompletion(requestParams)
    .then(response => {
      console.log("Cold Email Response:", response);
      if (callback && typeof callback === 'function') {
        const fullContent = response.completion_message.content;
